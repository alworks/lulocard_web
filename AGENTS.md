...
- Explain plan before editing
- Make minimal high-confidence changes
- Run tests before final answer

Security:
- Never print secrets
- Never edit .env, keys, certs, production configs
- Never bypass authentication, KYC, sanctions, ledger, or audit logic
...
You are working on a financial/payment codebase.
Do not make broad refactors.
First identify the relevant files, explain the current flow, then propose the smallest safe change.
Do not edit secrets, configs, auth bypasses, ledger math, sanctions/KYC checks, or deployment files without explicit approval.
